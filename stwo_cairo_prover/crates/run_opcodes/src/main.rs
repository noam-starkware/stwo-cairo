...
/// Command line arguments for run_opcodes.
/// Example command line (use absolute paths):
///     ```
///     cargo run -r --bin run_opcodes -- --run_from_cairo_pie
///     --opcodes_path path/to/opcodes_path --builtins_path path/to/builtins_path --secure_run=true
/// path/to/cairo/pie     ```
#[derive(Parser, Debug)]
...
struct Args {
    #[command(flatten)]
    vm_args: VmArgs,
    /// The output file path for the opcodes.
    #[structopt(long = "opcodes_path")]
    opcodes_path: PathBuf,
    #[structopt(long = "builtins_path")]
...
fn run(args: impl Iterator<Item = String>) -> Result<CairoProof<Blake2sMerkleHasher>, Error> {
    ...
    let opcodes_content = serde_json::to_string(&casm_states_by_opcode_count)?;
    let builtins_content = serde_json::to_string(&cairo_input.builtins_segments)?;

    let proof = prove_cairo::<Blake2sMerkleChannel>(cairo_input, prover_config)?;

    std::fs::write(args.opcodes_path, opcodes_content)?;
    std::fs::write(args.builtins_path, builtins_content)?;
